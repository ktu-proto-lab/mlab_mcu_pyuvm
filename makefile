...
ifeq ($(SIMULATOR),verilator)
	...
	@source $(PROJECT_ROOT)/uvm/script/logger.sh; \
	if [ "$(VIEW_TARGET)" = "coverage" ]; then \
		COVERAGE_DATA_FILE="sim/cov_work/scope/test/"; \
		COVERAGE_DATA_PATH="$(PROJECT_ROOT)/uvm/$$COVERAGE_DATA_FILE"; \
		if [ -d "$$COVERAGE_DATA_PATH" ]; then \
			imc -load "$$COVERAGE_DATA_PATH" >/dev/null 2>&1 & \
			logger SUCCESS "coverage generated by $(XCELIUM_NAME), opened on Cadence IMC"; \
			exit 0; \
		else \
			logger ERROR "coverage data folder not found at '$$COVERAGE_DATA_PATH'"; \
			exit 1; \
		fi; \
	fi; \
	if [ "$(VIEW_TARGET)" = "waves" ]; then \
