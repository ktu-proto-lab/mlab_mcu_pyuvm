...
from error import CliMemCmdUnknown

class CliMemCmdEnum(Enum):
    read = 1
    write = 2
    cksum = 3
    dump = 4
...
@vsc.randobj
class CliMemTransaction(uvm_sequence_item):    
    def __init__(self, name="CliMemTransaction"):
        super().__init__(name)
        self.cmd = vsc.rand_enum_t(CliMemCmdEnum)
        self.addr = vsc.rand_uint32_t()
        self.value = vsc.rand_int32_t()
    ...
    def c_cmd_write_value_dist(self):
        ...
        with vsc.if_then(self.cmd == CliMemCmdEnum.write):
    ...
    def c_word_count_range(self):
        with vsc.if_then((self.cmd == CliMemCmdEnum.cksum) | (self.cmd == CliMemCmdEnum.dump)):
            self.word_count in vsc.rangelist(vsc.rng(1, 16))
        
    @vsc.constraint
    def c_cmd_equal_dist(self):
        vsc.dist(self.cmd,[
            vsc.weight(CliMemCmdEnum.read, 20),
            vsc.weight(CliMemCmdEnum.write, 20),
            vsc.weight(CliMemCmdEnum.cksum, 20),
            vsc.weight(CliMemCmdEnum.dump, 20),
            vsc.weight(CliMemCmdEnum.test, 20)
        ])
        
    def to_string(self) -> str:
        if self.cmd == CliMemCmdEnum.read:
            return f"mem read {hex(self.addr)}"
        if self.cmd == CliMemCmdEnum.write:
            return f"mem write {hex(self.addr)} {hex(self.value)}"
        if self.cmd == CliMemCmdEnum.cksum:
            return f"mem cksum {hex(self.addr)} {hex(self.word_count)}"
        if self.cmd == CliMemCmdEnum.dump:
            return f"mem dump {hex(self.addr)} {hex(self.word_count)}"
        if self.cmd == CliMemCmdEnum.test:
            return "mem test"
        raise CliMemCmdUnknown(f"unhandled command {self.cmd}")
